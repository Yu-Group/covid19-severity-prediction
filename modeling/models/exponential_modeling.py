...
def exponential_fit(counts, mode, target_day=np.array([1])):
    ...
    for i in range(len(counts)):
        ...
        elif min(train_ts[start:]) > 0 and min(np.diff(np.log(train_ts[start:]))) == max(
                np.diff(np.log(train_ts[start:]))):
...
def fit_and_predict_shared_exponential(df, mode, outcome='deaths', demographic_vars=[], target_day=np.array([1]),
    ...
    predicted_deaths = get_shared_death_predictions(df, model, mode, target_day=target_day, outcome=outcome,
                                                    demographic_vars=demographic_vars, scaler=scaler)
...
def get_shared_death_predictions(test_df, model, mode, target_day, scaler, outcome='deaths', demographic_vars=[]):
    ...
    for i, deaths in enumerate(county_deaths):
        ...
        if mode == 'predict_future':
            ...
            predicted_county_deaths = _predict_shared_deaths(deaths[-(target_day[-1] + 1)], demographics, model,
                                                             target_day, scaler)
