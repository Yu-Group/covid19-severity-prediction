...
def prep_county_df(df, NUM_DAYS_LIST):
    ...
    days = ['Predicted Deaths by ' + (datetime.datetime.today() + datetime.timedelta(days=i)).strftime("%B %d")
            for i in NUM_DAYS_LIST]
    remap = {
        f'Predicted Deaths {i}-day': 
        'Predicted Deaths by ' + (datetime.datetime.today() + datetime.timedelta(days=i)).strftime("%B %d")
        for i in NUM_DAYS_LIST
    }
    dfc = df.rename(columns=remap).sort_values(by=days[0], ascending=False).round(decimals=1)
